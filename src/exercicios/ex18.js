...
function JurosComposto() {
    ...
    return (
        <div style={styles.container}>
            <h2 style={styles.heading}>Calculadora de Juros Compostos</h2>
            <input 
                type="number" 
            ...
            <input 
                type="number" 
                placeholder="Informe o período (anos):" 
                value={valor3} 
                onChange={(e) => setValor3(e.target.value)} 
                style={styles.input}
            />
            <div style={styles.buttonContainer}>
                <button onClick={calcula} style={styles.button}>Calcular</button>
            </div>
            <p style={styles.resultado}>O montante com juros compostos é: R$ {resultado.toFixed(2)}</p>
        </div>
    );
}
